      ...
      <Widget
        ...
        actions={
          <PopoverTrigger asChild>
            ...
            >
              <Arrows className="w-4 h-4 text-Main" />
            </Button>
          </PopoverTrigger>
        }
      >
        <TabsContent value="products">
          <div className="flex flex-col gap-2">
            {productsWidgetMock.products.map((product: TopProduct) => (
              <Card key={product.id} product={product} mode={filter} />
            ))}
          </div>
        </TabsContent>
        <TabsContent value="categories">
          <div className="flex flex-col gap-2">
            {productsWidgetMock.categories.map((product: TopProduct) => (
              <Card key={product.id} product={product} mode={filter} />
            ))}
          </div>
        </TabsContent>
        <TabsContent value="returns">
          <div className="flex flex-col gap-2">
            {productsWidgetMock.returns.map((product: TopProduct) => (
              <Card key={product.id} product={product} mode={filter} />
            ))}
          </div>
        </TabsContent>
